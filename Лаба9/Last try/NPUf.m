...
for i=1:kolnds
   ...
   if(abs(y-275)<=10^(-4) && x <= 424.5 && x>= 412.5)
...
for k=1:size(underload,2)-1
   ...
   pressure_top = 1000*9.8*(290 - (y+(ynxt-y)/2));
   pressure_bot = 1000*9.8*(290 - y);
   Forcebot = 1/2*(pressure_bot+pressure_top)*length/2;
   force(2*underload(k)) = force(2*underload(k)) - Forcebot*cos(atan(2/5));
   force(2*underload(k)+1) = force(2*underload(k)+1) + Forcebot*sin(atan(2/5));
   
   
   
   pressure_top = 1000*9.8*(290 - ynxt);
   pressure_bot = 1000*9.8*(290 - (y+(ynxt-y)/2));
   Forcetop = 1/2*(pressure_bot+pressure_top)*length/2;
   force(2*underload(k+1)) = force(2*underload(k+1)) - Forcetop*cos(atan(2/5));
   force(2*underload(k+1)-1) = force(2*underload(k+1)-1) + Forcetop*sin(atan(2/5));
...
for k=1:size(underload,2)-1
   ...
   pressure_bot = 1000*9.8*(290 - y);
   Forcebot = 1/2*(pressure_bot+pressure_top)*length/2;
   force(2*underload(k)) = force(2*underload(k)) - Forcebot*(cos(atan(2/3)));
   force(2*underload(k)-1) = force(2*underload(k)-1) + Forcebot*(sin(atan(2/3)));
   ...
   pressure_bot = 1000*9.8*(290 - (y+(ynxt-y)/2));
   Forcetop = 1/2*(pressure_bot+pressure_top)*length/2;
   force(2*underload(k+1)) = force(2*underload(k+1)) - Forcetop*(cos(atan(2/3)));
   force(2*underload(k+1)-1) = force(2*underload(k+1)-1) + Forcetop*(sin(atan(2/3))); 
end
